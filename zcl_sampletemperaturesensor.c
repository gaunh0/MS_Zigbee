...
static void zclSampleTemperatureSensor_SendState(OUTPUT_TypeDef OUTX)
{
	...
	// {
		...
		// osal_memcpy(pos,"OUT1",4);
	// }
	// else if (OUTX == 2)
	// {
		// temp_s = CheckOutputState(Output_2);
		// osal_memcpy(pos,"OUT2",4);
	// }
	// else if (OUTX == 3)
	// {
		// temp_s = CheckOutputState(Output_3);
		// osal_memcpy(pos,"OUT3",4);
	// }
	// state = BUILD_UINT8(0,1);
	// UART_SendNum(HAL_UART_PORT_0,temp_s);
	 UART_SendString(HAL_UART_PORT_0,test);
...
{ 
  ...
  pInReportCmd = (zclReportCmd_t *)pInMsg->attrCmd;
	...
	if (pInReportCmd->attrList[0].attrID == ATTRID_CONTROL_DATA)
	{
		...
		if (strstr(tmp_Data,"OUT1;1"))
		{
			TURN_on(Output_1);
		}
		else if (strstr(tmp_Data,"OUT2;1"))
		{
			TURN_on(Output_2);
		}
		else if (strstr(tmp_Data,"OUT3;1"))
		{
			TURN_on(Output_3);
		}
		else if (strstr(tmp_Data,"OUT1;0"))
		{
			TURN_off(Output_1);
		}
		else if (strstr(tmp_Data,"OUT2;0"))
		{
			TURN_off(Output_2);
		}
		else if (strstr(tmp_Data,"OUT3;0"))
		{
			TURN_off(Output_3);
		}
		else
