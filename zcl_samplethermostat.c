...
static void zclSampleThermostat_ProcessInReportCmd( zclIncomingMsg_t *pInMsg )
{
  ...
  {		
	  pOutDemandReport = (zclReportCmd_t *)outDemandBuffer;
		src_EndPoint = pInMsg->srcAddr.endPoint;	

		HalLedBlink ( HAL_LED_1, 1, 50, 500 );

	  // store the current temperature value sent over the air from temperature sensor
	  zclSampleThermostat_LocalTemperature = pInReportCmd->attrList[0].attrData[0] * 100;
	  ...
	  pOutDemandReport->attrList[1].dataType 	= ZCL_DATATYPE_UINT8;
		pOutDemandReport->attrList[2].attrID 		= ATTRID_ENDPOINT;
	  pOutDemandReport->attrList[2].dataType 	= ZCL_DATATYPE_UINT8;
		pOutDemandReport->attrList[2].attrData	= &src_EndPoint;
	  ...
	  if ( send )
	  {
	    zcl_SendReportCmd( SAMPLETHERMOSTAT_ENDPOINT, &zclSampleThermostat_DstAddr,
			...
			UART_DebugPrint(HAL_UART_PORT_0, "SENT TO ENGINE!");

			/* Feedback Data Sent */
			if (FLAG_ECHO_SDATA)
			{
				UART_ZCmdPrintString(HAL_UART_PORT_0, "@ZBS:");
				UART_ZCmdPrintString(HAL_UART_PORT_0, "indirect");															// destShortAddr
				UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
				UART_ZCmdPrintNum 	(HAL_UART_PORT_0, src_EndPoint);															// destEndPoint
				UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
				UART_ZCmdPrintNum 	(HAL_UART_PORT_0, zclSampleThermostat_HeatingDemand);					// destHeating
				UART_ZCmdPrintString(HAL_UART_PORT_0, ";");
				UART_ZCmdPrintNum 	(HAL_UART_PORT_0, zclSampleThermostat_CoolingDemand);					// destCooling
				UART_ZCmdPrint			(HAL_UART_PORT_0, "!");
			}	

			/* Send report to ZB Coordinator */
			#ifdef ROUTER	
			uint16 HeatingCooling_Demand = BUILD_UINT16(zclSampleThermostat_HeatingDemand, zclSampleThermostat_CoolingDemand);
			uint16 TempHum_Value = BUILD_UINT16(pInReportCmd->attrList[0].attrData[0], pInReportCmd->attrList[0].attrData[1]);
			
			afAddrType_t DstAddr;
			DstAddr.addrMode = (afAddrMode_t)Addr16Bit;
			DstAddr.endPoint = APP_COORDINATOR_ENDPOINT;
			...
			if ( pReportCmd != NULL )
			{
				...
				// Sensor value
				pReportCmd->attrList[0].attrID		= ATTRID_REPORT_DATA_COORD;
				pReportCmd->attrList[0].dataType	= ZCL_DATATYPE_INT16;
				pReportCmd->attrList[0].attrData	= (void *)(&TempHum_Value);
				// Heating / Cooling Demand
				pReportCmd->attrList[1].attrID		= ATTRID_REPORT_DATA_COORD;
				pReportCmd->attrList[1].dataType	= ZCL_DATATYPE_UINT16;
				pReportCmd->attrList[1].attrData	= (void *)(&HeatingCooling_Demand);
				// Information: shortAddr (16bits), endPoint (8bits)
				pReportCmd->attrList[2].attrID		= pInMsg->srcAddr.addr.shortAddr;
				pReportCmd->attrList[2].dataType	= ZCL_DATATYPE_UINT8;
				pReportCmd->attrList[2].attrData	= &(pInMsg->srcAddr.endPoint);
	...
	{	
		UART_SendString(HAL_UART_PORT_0,"innnnnnn");
		uint8 i;
		uint8 *tmp_pos;
		if (FLAG_ECHO_RDATA)
		{
			...
			for (i = 0; i < 5; i++)
			{
				tmp_pos[i+1] = pInReportCmd->attrList[1].attrData[i];
			}
			UART_ZCmdPrintBuffer(HAL_UART_PORT_0,tmp_pos,5);
			UART_ZCmdPrint		(HAL_UART_PORT_0, ";");
